...
get '/chat_messages' do
  ...
  user_ids = results.map{ |row| row["user_id"] }.uniq

  if user_id.count > 0
    users = client.query("SELECT id, avatar_file_name, first_name FROM users WHERE id IN (#{user_ids.join(', ')})").group_by{ |row| row["id"] }
